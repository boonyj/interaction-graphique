...
void placeur_runfunc(ei_widget_t widget){
        ...
        //if (&(placeur->rel_x) != NULL || &(placeur->rel_y) != NULL) {
        ei_widget_t parent = widget->parent;
        int parent_width = parent->screen_location.size.width;
        int parent_height = parent->screen_location.size.height;

        int parent_coordinate_x = (int)((float)parent_width * placeur->rel_x)+ parent->screen_location.top_left.x;
        int parent_coordinate_y = (int)((float)parent_height * placeur->rel_y)+ parent->screen_location.top_left.y;

        placeur->width = (placeur->rel_width != 0.0) ? (int)((float)parent_width * placeur->rel_width) : placeur->width;
        placeur->height = (placeur->rel_height != 0.0) ? (int)((float)parent_height * placeur->rel_height) : placeur->height;

        switch (placeur->anchor) {
                case ei_anc_center:
                        new_location.top_left.x =  placeur->x +parent_coordinate_x - placeur->width/2;
                        new_location.top_left.y =  placeur->y +parent_coordinate_y - placeur->height/2;
                        break;
                case ei_anc_north:
                        new_location.top_left.x =  placeur->x +parent_coordinate_x - placeur->width/2;
