...
void placeur_runfunc(ei_widget_t widget){
        ...
        if (widget->parent != NULL) {
                ...
                printf("Parent (%s): w = %d, h = %d\n",parent->wclass->name,parent->screen_location.size.width,parent->screen_location.size.height);
                printf("Parent topleft (%s): x = %d, y = %d\n",parent->wclass->name,parent->screen_location.top_left.x,parent->screen_location.top_left.y);
                ...
                placeur->width = (placeur->rel_width != 0.0) ? (int) ((float) parent_width * placeur->rel_width)
                                                             : placeur->width;
                placeur->height = (placeur->rel_height != 0.0) ? (int) ((float) parent_height * placeur->rel_height)
                                                               : placeur->height;
